import { createDocument, getDocuments, updateDocument } from './firebase';
...
export const getNotificationPreferences = async (userId: string): Promise<NotificationPreferences | null> => {
  try {
    const allPreferences = await getDocuments('notification_preferences');
    const preferences = allPreferences.filter(pref => pref.user_id === userId);
...
export const NotificationTemplates = {
  ...
  systemMaintenance: (maintenanceTime: string) => ({
    title: 'System Maintenance',
    message: `Scheduled maintenance on ${maintenanceTime}. System may be temporarily unavailable.`,
    type: 'info' as const,
...
export const sendBulkNotifications = async (
  ...
  notificationTemplate: ReturnType<typeof NotificationTemplates[keyof typeof NotificationTemplates]>
) => {
  try {
